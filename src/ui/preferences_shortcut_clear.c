...
bool bongo_cat_neo_pref_shortcut_clear(struct nk_context *context,
    ...
    float amount = bongo_cat_neo_ui_animate_eased(context, animation_id,
        hover ? 1.0f : 0.0f, 150, BONGO_CAT_NEO_UI_EASE_STANDARD);
    struct nk_color color = bongo_cat_neo_ui_color_alpha(
        bongo_cat_neo_ui_color_mix(p.muted, p.pink, amount), opacity);
