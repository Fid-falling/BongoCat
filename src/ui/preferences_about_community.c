...
static void community_link(BongoCatNeoPreferences *value,
    struct nk_context *context, struct nk_command_buffer *canvas,
    struct nk_rect bounds, int index, BongoCatNeoUIPalette p) {
    ...
    float lift = bongo_cat_neo_ui_animate_eased(context, id,
        hover ? 1.0f : 0.0f, 220.0f, BONGO_CAT_NEO_UI_EASE_SWIFT);
    bounds.y -= 2.0f * lift;
    struct nk_color background = bongo_cat_neo_ui_color_mix(p.surface_glass,
        brand, hover ? .10f : .055f);
    if (hover && p.effects) bongo_cat_neo_ui_paint_shadow(context, bounds, 18,
    ...
    bongo_cat_neo_preferences_icon_draw(value, canvas,
        index ? BONGO_CAT_NEO_UI_ICON_QQ : BONGO_CAT_NEO_UI_ICON_DISCORD,
...
static void coffee(BongoCatNeoPreferences *value, struct nk_context *context,
    ...
    if (hover) bongo_cat_neo_ui_cursor_hover_rect(context, pill,
        BONGO_CAT_NEO_UI_CURSOR_POINTER);
    if (hit(context, pill)) open_url("https://bongocatneo.com/support");
}
