...
bool bongo_cat_neo_pref_control_toggle(struct nk_context *context,
    ...
    float progress = bongo_cat_neo_ui_animate_eased(context, id,
        *value ? 1.0f : 0.0f, 250.0f, BONGO_CAT_NEO_UI_EASE_SPRING);
    char hover_id[80];
    ...
    if (state_amount > 0.0f && p.effects)
        bongo_cat_neo_ui_paint_shadow(context, track, 12, 0, 2, 8, 0,
            nk_rgba(p.accent.r, p.accent.g, p.accent.b,
            (nk_byte)(89 * state_amount)));
    nk_fill_rect(canvas, track, 12,
        bongo_cat_neo_ui_color_mix(p.field, p.accent, state_amount));
    nk_stroke_rect(canvas, track, 12, 1.0f, bongo_cat_neo_ui_color_mix(
    ...
    if (p.effects) bongo_cat_neo_ui_paint_shadow(context, knob,
