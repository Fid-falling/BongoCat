...
static bool packed_files_present(const unsigned char *data, size_t size,
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if (!safe_name(name) || !bongo_cat_neo_path_join(path, sizeof(path), target, name) ||
            !bongo_cat_neo_path_file_size(path, &actual_size) ||
...
static bool extract_pack(const unsigned char *data, size_t size, const char *target,
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if (slash) {
            ...
            if (!bongo_cat_neo_path_create_directory(parent)) return false;
        }
...
BongoCatNeoResult bongo_cat_neo_platform_embedded_assets(const char *target, BongoCatNeoError *error) {
    ...
    if (!data || !size || !bongo_cat_neo_path_create_directory(target) ||
        !extract_pack(data, size, target, error)) {
        if (error && !error->message[0]) bongo_cat_neo_error_set(error,
            BONGO_CAT_NEO_ERROR_IO, "Cannot rebuild the embedded asset cache: %s",
            SDL_GetError());
        return BONGO_CAT_NEO_ERROR_IO;
    }
