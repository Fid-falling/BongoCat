...
static bool required_assets(const char *root) {
    ...
    for (size_t i = 0; i < sizeof(files) / sizeof(files[0]); ++i) {
        ...
        if (!bongo_cat_neo_path_join(path, sizeof(path), root, files[i]) ||
            !bongo_cat_neo_path_is_file(path)) return false;
    }
    return true;
}
...
static bool asset_directory(char *output, size_t capacity, const char *root) {
    return bongo_cat_neo_path_join(output, capacity, root, "assets") &&
        bongo_cat_neo_path_is_dir(output) && required_assets(output);
}

BongoCatNeoResult bongo_cat_neo_app_locate_assets(BongoCatNeoApp *app,
    ...
    if (!asset_directory(app->asset_root, sizeof(app->asset_root), base ? base : "")) {
        ...
        if (bongo_cat_neo_path_join(cache, sizeof(cache), app->data_root, name)) {
            BongoCatNeoError embedded = {0};
            if (bongo_cat_neo_platform_embedded_assets(cache, &embedded) == BONGO_CAT_NEO_OK) {
...
#ifndef NDEBUG
    if (!required_assets(app->asset_root) && required_assets(BONGO_CAT_NEO_DEV_ASSET_ROOT))
