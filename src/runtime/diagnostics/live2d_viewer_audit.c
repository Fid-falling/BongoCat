...
static bool viewer_timing(const char *path, int *track, int *returning) {
...
bool bongo_cat_live2d_viewer_audit_run(BongoCatApp *app) {
    ...
    bool expression_parameter = parameter(app, "Param6", &expression_before);
    if (bongo_cat_live2d_set_expression(app->live2d, 2)) {
        ...
        passed = expression_parameter && parameter(app, "Param6",
    ...
    BongoCatParameterRange angle_x_range, angle_y_range;
    ...
    const float target_eye_y = viewer_target(&eye_y_range,
        target_drag_y, viewer_gain);
    bongo_cat_live2d_set_dragging(app->live2d, target_drag_x, target_drag_y);
    float previous_x = 0.0f, maximum_step = 0.0f;
    int track_frames[] = {1, 2, 4, 8, 15, 30};
    int return_frames[] = {1, 2, 4, 8, 15, 30};
    passed = viewer_timing(app->smoke_viewer_trace, track_frames,
    ...
    previous = 0;
    for (size_t i = 0; i < 6; ++i) {
        passed = advance_tracking(app, return_frames[i] - previous,
            &previous_x, &maximum_step) && passed;
