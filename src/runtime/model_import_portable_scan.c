...
typedef struct PortableScan {
    BongoCatNeoPortableVisitor visitor;
    void *userdata;
...
static BongoCatNeoPathVisit collect_child(void *userdata,
    const char *dirname, const char *name) {
    ChildList *list = userdata;
    if (++list->entries > PORTABLE_ENTRY_CAP || list->count >= PORTABLE_CHILD_CAP) {
        list->limited = true; return BONGO_CAT_NEO_PATH_SUCCESS;
    }
    if (name[0] == '.') return BONGO_CAT_NEO_PATH_CONTINUE;
    char path[BONGO_CAT_NEO_PATH_CAP];
    if (bongo_cat_neo_path_join(path, sizeof(path), dirname, name) &&
        bongo_cat_neo_path_is_dir(path))
...
static BongoCatNeoResult scan_node(PortableScan *scan, const char *source, int depth) {
    if (!within_budget(scan)) return BONGO_CAT_NEO_OK;
    PortableWorkspace *work = calloc(1, sizeof(*work));
    if (!work) {
        bongo_cat_neo_error_set(scan->error, BONGO_CAT_NEO_ERROR_MEMORY,
            "Cannot allocate portable model scan workspace");
        return BONGO_CAT_NEO_ERROR_MEMORY;
    }
    BongoCatNeoError local = {0};
    int found = bongo_cat_neo_import_mver_discover_exact(source, &work->discovery, &local);
    if (found <= 0) {
        memset(&work->discovery, 0, sizeof(work->discovery)); local = (BongoCatNeoError){0};
        found = bongo_cat_neo_import_mver_patch_discover(source, &work->discovery, &local);
    }
    if (found > 0) {
        BongoCatNeoResult result = scan->visitor(scan->userdata, source,
            &work->discovery, scan->error);
        free(work); return result;
    }
    BongoCatNeoResult result = BONGO_CAT_NEO_OK;
    if (depth < PORTABLE_SCAN_DEPTH &&
        bongo_cat_neo_path_enumerate(source, collect_child, &work->children)) {
...
BongoCatNeoResult bongo_cat_neo_import_portable_scan(const char *root,
    ...
    if (!root || !visitor) return BONGO_CAT_NEO_ERROR_ARGUMENT;
    ChildList *children = calloc(1, sizeof(*children));
    if (!children) return BONGO_CAT_NEO_ERROR_MEMORY;
    PortableScan scan = {visitor, userdata, error, 0,
        SDL_GetTicksNS() + PORTABLE_SCAN_BUDGET_NS, false};
    BongoCatNeoResult result = BONGO_CAT_NEO_OK;
    if (bongo_cat_neo_path_enumerate(root, collect_child, children)) {
